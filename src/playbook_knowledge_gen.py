...
def calculate_similarity(
    account_description, industry_descriptions, persona_descriptions
):
    """
    ...
    industry_similarities = cosine_similarity([account_embedding], industry_embeddings)[
        0
    ]
...
def process_account_target(target: Dict[str, str]):
    if target["target_type"] == "Accounts":
        account_url = target.get(
            "context", ""
        )  # Assuming the URL is stored in 'context'
        if account_url:
            print(
                f"Extracting knowledge for account: {target['name']} from {account_url}"
            )
    ...
    for target in targets:
        ...
        if target_type == "Accounts":
            if context:  # Ensure there's a URL
                ...
                account_knowledge = parse_account_url(
                    context
                )  # Assuming this function extracts knowledge from the URL

                # Include the account name in the content to calculate similarity
                account_knowledge_with_name = (
                ...
                with open(log_file_path, "a", encoding="utf-8") as log_file:
                    ...
                    log_file.write(
                        f"Most Similar Industries: {most_similar_industry}\n"
                    )
