...
def generate_marketing_pitch(account_info, industry_descriptions, persona_descriptions):
    ...
    final_instruction = (
        ...
        "Generate Marcketing material in arround 2000 words."
    )
...
def replacement_content_gen(account_info, industry_descriptions, persona_descriptions, positions, input_html_path):
    account_name = account_info['name']
    account_url = account_info['context']
...
def replacement_content_gen_with_pitch(account_info, industry_descriptions, persona_descriptions, positions, input_html_path):
    """
    ...
    marketing_pitch = generate_marketing_pitch(account_info, industry_descriptions, persona_descriptions)
    ...
    for t in tag_infos:
        ...
        final_instruction += f"  Text content to replace: \"{t['original']}\" (about {t['length']} characters)\n"

    # Step 4: Create the conversation for GPT (including the SystemMessage and HumanMessage)
    conversation = [
        SystemMessage(content="You are an expert in marketing content generation."),
        HumanMessage(content=f"Marketing Pitch for {account_info['name']}: {marketing_pitch}"),
        ...
        if target:
            ...
            if isinstance(target.string, NavigableString):
                ...
                try:
                    ...
                    print(f"Warning: Could not parse replacement for {key}. Falling back to plain text.")
